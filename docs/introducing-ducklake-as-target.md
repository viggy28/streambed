...
The [small-file problem](https://www.dremio.com/blog/apache-iceberg-small-files-problem-causes-fixes-and-prevention/) is well known in the lakehouse world but it gets worse for CDC workload.

Every flush to lakehouse has a cost. It create more files, more metadata, and more commit work. Updates and deletes make it worse, especially with copy-on-write.
